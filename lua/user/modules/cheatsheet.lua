...
local mappings_tb = require("user.mappings")
-- local isValid_mapping_TB = require("user.modules.cheatsheet").isValid_mapping_TB
...
local isValid_mapping_TB = function(tbl)
  local isEmpty = true
...
return function()
  ...
  local highlights_raw = vim.split(vim.api.nvim_exec("filter " .. "CheatsheetHeader" .. " hi", true), "\n")
  local highlight_groups = {}

  for _, raw_hi in ipairs(highlights_raw) do
    table.insert(highlight_groups, string.match(raw_hi, "CheatsheetHeader" .. "%S+"))
  end
    ...
    for column_i, _ in ipairs(columns) do
      ...
      if columns[column_i][i] then
        ...
        if vim.tbl_contains(card_headings, columns[column_i][i]) then
          ...
          -- require"user.utils".tprint(columns)
          vim.api.nvim_buf_add_highlight(
            buf,
            nvcheatsheet,
  ...
  vim.keymap.set("n", "<ESC>", function()
    require("user.modules.tabline").close_buffer(buf)
